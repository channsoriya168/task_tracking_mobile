...
class TaskViewHeaderCard extends StatelessWidget {
  const TaskViewHeaderCard({
    super.key,
    required this.task,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _TaskViewStatusBadge(task: task),
              if (task.dueDate != null) ...[
                const SizedBox(width: 10),
                _TaskViewDueDateLabel(task: task, isDark: isDark),
              ],
            ],
          ),
        ],
...
class _TaskViewStatusBadge extends StatelessWidget {
  const _TaskViewStatusBadge({required this.task});
  final TaskModel task;
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            height: 6,
            decoration:
                BoxDecoration(color: task.statusColor, shape: BoxShape.circle),
          ),
          const SizedBox(width: 5),
          Text(
            task.statusLabel,
...
class _TaskViewDueDateLabel extends StatelessWidget {
  const _TaskViewDueDateLabel({required this.task, required this.isDark});
  final TaskModel task;
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    final color =
        task.isOverdue ? kHighPriority : (isDark ? Colors.white38 : kTextMuted);
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Icon(
        ...
        Text(
          ...
          style: TextStyle(
            fontSize: 12,
            fontWeight:
                task.isOverdue ? FontWeight.w600 : FontWeight.w500,
            color: color,
          ),
        ),
      ],
